...
LightAlbedoShader::LightAlbedoShader(ID3D11Device* device, HWND hwnd) : BaseShader(device, hwnd), matrixDataModule(device, hwnd), lightsDataModule(device, hwnd)
{
	initShader(L"light_vs.cso", L"lightAlbedo_ps.cso");
}


LightAlbedoShader::~LightAlbedoShader()
{
	// Release the layout.
	if (layout)
	{
...
void LightAlbedoShader::initShader(const wchar_t* vsFilename, const wchar_t* psFilename)
{
...
void LightAlbedoShader::setShaderParamaters(
